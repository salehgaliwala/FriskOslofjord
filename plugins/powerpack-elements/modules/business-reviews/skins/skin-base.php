...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	protected function register_content_review_details_controls() {
		...
		$this->start_controls_section(
			...
			array(
				'label' => esc_html__( 'Review Details', 'powerpack' ),
			...
			$this->add_control(
				...
				array(
					'label'     => esc_html__( 'Reviewer Image', 'powerpack' ),
			...
			$this->add_control(
				...
				array(
					...
					'label_off'    => esc_html__( 'Now', 'powerpack' ),
					'return_value' => 'yes',
					'default'      => 'yes',
			...
			$this->add_control(
				...
				array(
					...
					'default'   => 'left',
					'options'   => array(
						'top'      => esc_html__( 'Above Name', 'powerpack' ),
						'left'     => esc_html__( 'Left of Name', 'powerpack' ),
						'all_left' => esc_html__( 'Left of all content', 'powerpack' ),
					),
					'condition' => array(
			...
			$this->add_control(
				'overall_align',
				array(
					'label'        => esc_html__( 'Overall Alignment', 'powerpack' ),
			...
			$this->add_control(
				...
				array(
					'label'     => esc_html__( 'Reviewer Name', 'powerpack' ),
					'type'      => Controls_Manager::HEADING,
					'separator' => 'before',
				)
			);
			...
			$this->add_control(
				'reviewer_name',
				array(
					'label'        => esc_html__( 'Show Name', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => esc_html__( 'Yes', 'powerpack' ),
					'label_off'    => esc_html__( 'Now', 'powerpack' ),
					'return_value' => 'yes',
			...
			$this->add_control(
				'reviewer_name_link',
				array(
					'label'        => esc_html__( 'Link Name', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => esc_html__( 'Yes', 'powerpack' ),
			...
			$this->add_control(
				'star_rating',
				array(
					'label'        => esc_html__( 'Show Star Rating', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'return_value' => 'yes',
					'default'      => 'yes',
				)
			...
			$this->add_control(
				...
				array(
					'label'     => esc_html__( 'Review Source Icon', 'powerpack' ),
					'type'      => Controls_Manager::HEADING,
					'separator' => 'before',
				)
			);
			...
			$this->add_control(
				'review_source_icon',
				array(
					'label'        => esc_html__( 'Show Review Source Icon', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'return_value' => 'yes',
					'default'      => 'yes',
					'render_type'  => 'template',
			...
			$this->add_control(
				...
				array(
					'label'     => esc_html__( 'Review Date', 'powerpack' ),
					'type'      => Controls_Manager::HEADING,
					'separator' => 'before',
				)
			);
			...
			$this->add_control(
				'review_date',
				array(
					'label'        => esc_html__( 'Show Review Date', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'return_value' => 'yes',
					'default'      => 'yes',
				)
			...
			$this->add_control(
				'review_date_type',
				array(
					'label'     => esc_html__( 'Select Type', 'powerpack' ),
					'type'      => Controls_Manager::SELECT,
					'default'   => 'relative',
					'options'   => array(
						'default'  => esc_html__( 'Numeric', 'powerpack' ),
						'relative' => esc_html__( 'Relative', 'powerpack' ),
					),
					'condition' => array(
			...
			$this->add_control(
				'review_content',
				array(
					'label'        => esc_html__( 'Show Review Text', 'powerpack' ),
					'type'         => Controls_Manager::SWITCHER,
					'return_value' => 'yes',
					'default'      => 'yes',
				)
			...
			$this->add_control(
				...
				array(
					'label'     => esc_html__( 'Text Length', 'powerpack' ),
			...
			$this->add_control(
				...
				array(
					...
					'raw'             => esc_html__( 'Yelp API allows fetching maximum 160 characters from a review.', 'powerpack' ),
	...
	protected function register_style_rating_controls() {
		$this->start_controls_section(
			...
			$this->add_control(
				'star_style',
				array(
					'label'     => esc_html__( 'Star Icon Style', 'powerpack' ),
					'type'      => Controls_Manager::SELECT,
					'default'   => 'custom',
					'options'   => array(
	...
	public function render_stars( $total_rating, $review, $settings ) {
		...
		if ( 'default' === $this->get_instance_value( 'star_style' ) ) {
			...
			if ( 'google' === $review['source'] ) {
				$marked_icon_html   = $this->render_star_icon('star-solid', 'pp-star-full pp-star-default');
				$unmarked_icon_html = $this->render_star_icon('star-solid', 'pp-star-empty pp-star-default');
				$flag               = 1;
			} else {
...
<g>
	...
	<path fill="#CECECE" d="M30.43,0.055h-4.505c-0.3,0-0.563,0.252-0.563,0.571v4.497c0,0.314,0.266,0.572,0.563,0.572h4.505
		...
		C30.371,2.302,30.416,2.428,30.34,2.486z" class="pp-yelp-rating-5"/>
			...
			$marked_icon_html   = $this->render_star_icon( 'star-solid', 'pp-star-full pp-star-custom' );
			$unmarked_icon_html = $this->render_star_icon( $star_icon, 'pp-star-empty pp-star-custom' );
			$flag               = 1;
		}
	...
	public function get_api_data( $source ) {
		...
		if ( 'google' === $source ) {
			...
			if ( empty( $api_key ) ) {
				return new \WP_Error( 'missing_api_key', esc_html__( 'To display Google Reviews, you need to setup API key.', 'powerpack' ) );
			}
			if ( empty( $place_id ) ) {
				return new \WP_Error( 'missing_place_id', esc_html__( 'To display Google Reviews, you need to provide valid Place ID.', 'powerpack' ) );
			}
		...
		if ( 'yelp' === $source ) {
			...
			if ( empty( $business_id ) ) {
				return new \WP_Error( 'missing_business_id', esc_html__( 'To display Yelp Reviews, you need to provide valid Business ID.', 'powerpack' ) );
			}
		...
		if ( empty( $response['data'] ) ) {
			...
			if ( is_wp_error( $api_data ) ) {
				...
				if ( 200 === wp_remote_retrieve_response_code( $api_data ) ) {
					...
					if ( 'OK' !== $data->status ) {
						$response['error'] = isset( $data->error_message ) ? $data->error_message : esc_html__( 'No reviews found.', 'powerpack' );
					} else {
						if ( isset( $data->result ) && isset( $data->result->reviews ) ) {
							...
							$response['error'] = esc_html__( 'This place doesn\'t have any reviews.', 'powerpack' );
						}
					}
				}
			}	
				...
				if ( 200 !== wp_remote_retrieve_response_code( $api_data ) ) {
					...
					if ( isset( $data->error ) ) {
						if ( 'VALIDATION_ERROR' === $data->error->code ) {
							$response['error'] = esc_html__( 'Yelp Reviews Error: Invalid or empty API key.', 'powerpack' );
						}
						if ( 'BUSINESS_NOT_FOUND' === $data->error->code ) {
							$response['error'] = esc_html__( 'Yelp Reviews Error: Incorrect or empty Business ID.', 'powerpack' );
						}
						if ( 'INTERNAL_SERVER_ERROR' === $data->error->code ) {
							$response['error'] = esc_html__( 'Yelp Reviews Error: Something is wrong with Yelp.', 'powerpack' );
						}
					} else {
						$response['error'] = esc_html__( 'Yelp Reviews Error: Unknown error occurred.', 'powerpack' );
					}
				} else {
					$data = json_decode( wp_remote_retrieve_body( $api_data ) );

					if ( empty( $data ) || ! isset( $data->reviews ) || empty( $data->reviews ) ) {
						$response['error'] = esc_html__( 'This business doesn\'t have any reviews.', 'powerpack' );
					} else {
			...
			<?php if ( 'yes' === $this->get_instance_value( 'review_source_icon' ) && ( 'all_left' === $this->get_instance_value( 'image_align' ) || 'left' === $this->get_instance_value( 'image_align' ) ) ) { ?>
				<div class="pp-review-icon-wrap">
					<?php if ( 'yelp' === $review['source'] ) {
						$yelp_icon = '<span class="pp-icon">' . $this->render_network_icon('yelp') . '</span>';
	...
	private function render_star_icon( $network_name, $icon_class = '' ) {

		return $this->render_network_icon( $network_name, [ 'class' => 'pp-rating-star ' . $icon_class ] );
	}

	private function render_network_icon( $network_name, $attributes = [] ) {
		...
		if ( PP_Helper::is_feature_active( 'e_font_icon_svg' ) ) {
			...
			$this->parent->add_render_attribute( 'star-icon',
				[
					'class' => $network_icon_data['value'],
					'aria-hidden' => 'true',
				]
			);

			if ( isset( $attributes['class'] ) ) {
				$this->parent->add_render_attribute( 'star-icon', 'class', $attributes['class'] );
			}

			$icon = sprintf( '<i %s></i>', $this->parent->get_render_attribute_string( 'star-icon' ) );
		}
	...
	public function render() {
		...
		$layout_class = ( 'carousel' === $settings['layout'] ) ? 'pp-reviews-layout-carousel pp-swiper-slider swiper' : 'elementor-grid pp-reviews-layout-grid';
