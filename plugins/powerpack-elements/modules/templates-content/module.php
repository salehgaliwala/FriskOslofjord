...
class Module extends Module_Base {
	...
	protected static function empty_templates_message( $template_type = '' ) {
		return '<div id="elementor-widget-template-empty-templates">
				...
				<div class="elementor-widget-template-empty-templates-title">' . sprintf( esc_html__( 'You Haven’t Saved %sTemplates Yet.', 'powerpack' ), ucfirst( $template_type ) . ' ' ) . '</div>
				<div class="elementor-widget-template-empty-templates-footer">' . esc_html__( 'Want to learn more about Elementor library?', 'powerpack' ) . ' <a class="elementor-widget-template-empty-templates-footer-url" href="https://go.elementor.com/docs-library/" target="_blank">' . esc_html__( 'Click Here', 'powerpack' ) . '</a>
				</div>
				</div>';
	}
	...
	public static function render_template_content( $template_id, \PowerpackElements\Base\Powerpack_Widget $widget, $in_loop = false ) {
		...
		if ( ! $template_id ) {
			if ( method_exists( $widget, 'render_editor_placeholder' ) ) {
				$placeholder = esc_html__( 'Choose a post template that you want to use as post skin in widget settings.', 'powerpack' );

				$widget->render_editor_placeholder([
					'title' => esc_html__( 'No template selected!', 'powerpack' ),
					'body' => $placeholder,
				]);
			} else {
