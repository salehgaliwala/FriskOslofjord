...
class Advanced_Menu extends Powerpack_Widget {
	...
	protected function register_style_menu_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .pp-advanced-menu--main .pp-menu-item .sub-arrow' => 'color: {{VALUE}}',
					'{{WRAPPER}} .pp-advanced-menu--main .pp-menu-item .sub-arrow svg' => 'fill: {{VALUE}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '',
				'selectors' => array(
					'{{WRAPPER}} .pp-advanced-menu--main .pp-menu-item:hover .sub-arrow' => 'color: {{VALUE}}',
					'{{WRAPPER}} .pp-advanced-menu--main .pp-menu-item:hover .sub-arrow svg' => 'fill: {{VALUE}}',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .pp-advanced-menu .sub-arrow svg, .pp-advanced-menu__container.pp-menu-{{ID}} .sub-arrow svg' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};',
				),
				'condition' => array(
					'submenu_icon[value]!' => '',
				),
	...
	public function get_frontend_settings() {
		...
		if ( 'svg' === $submenu_icon['library'] ) {
			$icon_content = Icons_Manager::render_uploaded_svg_icon( $submenu_icon['value'] );
			$frontend_settings['submenu_icon']['rendered_tag'] = $icon_content;
		} else {
			// If the saved value is FA4, but the user has upgraded to FA5, the value needs to be converted to FA5.
			if ( 'fa ' === substr( $submenu_icon['value'], 0, 3 ) && Icons_Manager::is_migration_allowed() ) {
				$submenu_icon['value'] = str_replace( 'fa ', 'fas ', $submenu_icon['value'] );
			}
	...
	protected function render() {
		...
		$this->add_render_attribute(
			...
			array(
				...
				esc_attr( $menu_wrapper_classes ),
			)
		);

		if ( \Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode() ) {
			$icon_content = '';
			if ( 'svg' === $settings['submenu_icon']['library'] ) {
				$icon_content = Icons_Manager::render_uploaded_svg_icon( $settings['submenu_icon']['value'] );
			}

			$this->add_render_attribute( 'main-menu-wrapper', 'data-subicon-indicator', esc_attr( $icon_content ) );
		}
		?>
		<?php do_action( 'ppe_before_advanced_menu_wrapper' ); ?>
		<div <?php $this->print_render_attribute_string( 'main-menu-wrapper' ); ?>>
		<?php
